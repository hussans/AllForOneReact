...
import { getMexicanRestaurant } from '../services/DataService';

const MexicanRes = () => {
    const [message, setMessage] = useState('Click the button to get a Mexican restaurant!');

    const handleGetRestaurant = async () => {
        try {
            const result = await getMexicanRestaurant();
            setMessage(result);
        } catch (error) {
